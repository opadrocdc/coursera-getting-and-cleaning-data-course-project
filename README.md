...
To comply with the **Coursera Getting and Cleaning Data** course project requirements, The original dataset train and test data that was merged. Then, only the mean and standard deviation variables were selected. Labels were added to the observations and a final mean was calculated for each study subject and label for the selected variables. Below please find the variable names used in the final tidy dataset `df.txt`. You can read that dataset using the `read.table` operation. Additional descriptions for the variables naming convention can be found in the original dataset at the above URL or in this repository data folder: `/data/uci-har-dataset/`
